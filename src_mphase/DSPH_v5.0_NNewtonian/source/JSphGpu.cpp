...
void JSphGpu::ParticlesDataUp(unsigned n,const tfloat3 *boundnormal){
  ...
  if(MultiPhase)hipMemcpy(AuxNNg,AuxNN,sizeof(float)*n,hipMemcpyHostToDevice); //<vs_non-Newtonian>
...
unsigned JSphGpu::ParticlesDataDown(unsigned n,unsigned pini,bool code,bool onlynormal){
  ...
  //if (aux)hipMemcpy(AuxNN, AuxNNg + pini, sizeof(float)*n, hipMemcpyDeviceToHost);
...
void JSphGpu::InitRunGpu(){
  ...
  if(!MultiPhase && TVisco==VISCO_LaminarSPS)hipMemset(SpsTaug,0,sizeof(tsymatrix3f)*Np); //<vs_non-Newtonian>
...
void JSphGpu::PreInteractionVars_Forces(unsigned np,unsigned npb){
  ...
  //<vs_non-Newtonian>
  if(ViscEtaDtg)hipMemset(ViscEtaDtg,0,sizeof(float)*np);             //ViscEtaDtg[]=0 //<vs_non-Newtonian>
  if(SpsGradvelg)hipMemset(SpsGradvelg,0,sizeof(tsymatrix3f)*np);    //SpsGradvelg[]=(0,0,0,0,0,0). 
  if(SpsTaug)	hipMemset(SpsTaug,0,sizeof(tsymatrix3f)*np);
  if(D_tensorg)hipMemset(D_tensorg,0,sizeof(tsymatrix3f)*np);
  if(Visco_etag)hipMemset(Visco_etag,0,sizeof(float)*np);
...
void JSphGpu::PreInteraction_Forces(){
  ...
  VelMax=sqrt(velmax);
  hipMemset(ViscDtg,0,sizeof(float)*Np);           //ViscDtg[]=0
  //hipMemset(ViscEtaDtg, 0, sizeof(float)*Np);      //ViscEtaDtg[]=0  //<vs_non-Newtonian>
  ViscDtMax=0;
